...
class VectorStore:
    ...
    def _get_table(self):
        ...
        if self._table is None:
            try:
                ...
                self._table = self.db.create_table("embeddings", schema=self._make_schema())
        return self._table
    ...
    def add(self, entries: list[VectorEntry]) -> None:
        ...
        data = [{"id": e.id, "text": e.text, "vector": e.vector, "metadata": json.dumps(e.metadata)} for e in entries]
        table.add(data)
